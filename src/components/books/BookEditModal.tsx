...
import { sanitizeBookCategories } from "../../utils/books";
...
export function BookEditModal({ open, language, bookToEdit, onClose, onSave }: BookEditModalProps) {
    ...
    const handleSave = () => {
        ...
        const categories = sanitizeBookCategories(categoriesStr.split(","));
        onSave(
            title.trim(),
            ...
            <div className="modal panel-content glass-panel">
                ...
                >
                    <div className="form-group">
                        <label className="text-secondary">{tr(language, "books.bookTitle")}</label>
                        <input
                            type="text"
                    ...
                    <div className="form-group mt-4">
                        <label className="text-secondary">{tr(language, "books.statusLabel")}</label>
                        <div className="status-toggle-group mt-2 flex gap-2">
                            {(["want_to_read", "reading", "finished"] as BookStatus[]).map((nextStatus) => (
                                <button
                                    key={nextStatus}
                                    type="button"
                                    className={`glass-button flex-1 py-1 ${status === nextStatus ? "active aura-glow" : ""}`}
                                    onClick={() => setStatus(nextStatus)}
                                >
                                    {tr(language, `books.status.${nextStatus}`)}
                                </button>
                            ))}
                        </div>
                    </div>
